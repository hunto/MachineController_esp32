...
int uartStatus = UART_FREE;

char *UART_TAG = "uart_events";
        ...
        if(xQueueReceive(uart0_queue, (void * )&event, (portTickType)portMAX_DELAY)) {
            ...
            switch(event.type) {
                ...
                case UART_DATA:
                    ...
                    ESP_LOGI(UART_TAG, "data, len: %d; buffered len: %d; \nbuffered str: ", event.size, buffered_size);
                    break;
                ...
                case UART_PATTERN_DET:
                    ESP_LOGI(UART_TAG, "uart pattern detected\n");
                    break;
                //Others
                default:
...
void uart_data_handler(uint8_t* data) {
    // first, send the data to tcp client
    switch (uartStatus) {
        case UART_FREE:
            break;
        case UART_RECEIVE_DATA: {
            extern DataStructure head;
            DataStructure add = createDataStructure(data);
            listAdd(head, add);
            break;
        }
